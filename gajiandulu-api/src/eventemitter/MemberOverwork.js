...
class MemberOverwork {
  constructor(observable) {
    this.observable = observable;
  }

  listenMemberOverwork() {
    this.observable.addListener(EVENT.MEMBER_OVERWORK, async data => {
      ...
      const HEADING_MESSAGE = `${employeeData.user.full_name} melakukan lembur hari ini`;
      const BODY_MESSAGE = `${employeeData.user.full_name} melakukan lembur ${
        data.overwork
      } jam di tanggal ${data.presenceDate}`;
