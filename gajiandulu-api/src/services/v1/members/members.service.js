...
const memberService = {
        ...
        if (withdrawData) {
          return res
            ...
            .json(
              response(
                false,
                `Tidak dapat mengubah gaji bulanan karena ${
                  userData.full_name
                } telah melakukan withdraw bulan ini.`
              )
            );
        }
      }
  ...
  delete: async (req, res) => {
    ...
    try {
      ...
      const description = `${userTable.full_name} telah menghapus seorang anggota bernama ${
      ...
      return res.status(200).json(response(true, 'Member berhasil dihapus'));
    } catch (error) {
      if (error.errors) {
        return res.status(400).json(response(false, error.errors));
      }
