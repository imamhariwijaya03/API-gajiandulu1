...
const fs = require('fs');
const { response, presenceOverdueCheck, nodemailerMail, mailTemplates } = require('@helpers');
const {
...
const presenceService = {
  ...
  delete: async (req, res) => {
    ...
    try {
      ...
      presence = await Presence.findAll({ where: { id: presenceId } });
      if (!presence.length) {
      ...
      presence.forEach(val => {
        employeeId.push(val.employee_id.toString());
      });
      ...
      await EmployeeNote.destroy({ where: { employee_id: employeeId, date: presenceDate } });
