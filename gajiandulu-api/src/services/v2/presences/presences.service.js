  ...
  create: async (req, res) => {
    ...
    try {
      ...
      if (req.query.type) {
        if (req.query.type === 'leave' || req.query.type === 'holiday') {
          ...
          if (warningMessage.length) {
            ...
            const rangedDate = dateProcessor.getRangedDate(companyData.setting.payroll_date);
            responseMessage = `Berhasil membuat presensi manual ${type}. Anggota ${memberList} tidak mendapatkan gaji karena tidak ada jadwal sama sekali untuk ${
      ...
      if (warningMessage.length) {
        ...
        const memberList = warningMessage.toString().replace(/,/g, ', ');
        const rangedDate = dateProcessor.getRangedDate(companyData.setting.payroll_date);
        responseMessage = `Berhasil membuat presensi manual. Anggota ${memberList} tidak mendapatkan gaji karena tidak ada jadwal sama sekali untuk ${
          warningMessage.length === 1 ? 'dia' : 'mereka'
        } di rentang tanggal ${rangedDate.dateStart} s/d ${rangedDate.dateEnd}`;
      }
