...
const memberService = {
  createMember: async (req, res) => {
    ...
    try {
      ...
      if (emailExist) {
        ...
        const description = `${userData.full_name} telah mengundang seorang anggota baru bernama ${
          data.name
        }`;
  ...
  respondMember: async (req, res) => {
    ...
    try {
      const employee = await Employee.findOne({
        ...
        include: [{ model: User, attributes: ['id'] }, { model: Company, attributes: ['id'] }]
      });
      if (!employee) {
  ...
  getDetail: async (req, res) => {
    ...
    try {
      ...
      for (let i = 1; i <= arrayDate.length; i++) {
        ...
        if (!schedule.length) {
          schedule = await definedSchedulesHelper(arrayDate[i], employee.id);
        }
  ...
  editMember: async (req, res) => {
    ...
    try {
      ...
      // SEND NOTIFICATION TO MANAGERS
      const description = `${userData.full_name} telah mengedit data seorang anggota bernama ${
