  ...
  export: async (req, res) => {
    ...
    try {
      ...
      for (let i = 0; i <= presencesData.length - 1; i++) {
        ...
        let credit = 0;
        const scheduleArray = [];
        for (let d = 0; d < arrayDate.length; d++) {
          let schedule = [];
          ...
          if (!schedule.length) {
            schedule = await definedSchedulesHelper(arrayDate[d], presencesData[i].id);
          }
          if (schedule.length) {
            const compose = schedule[0];
            Object.assign(compose.dataValues, { date: arrayDate[d] });
            scheduleArray.push(compose);
          }
        }
        ...
        const firstHeaderContent = [
          ...
          'Hari Kerja Sebulan: ',
          `${presencesData[i].workdays || scheduleArray.length} Hari`,
          'Peran',
          roleName,
          ...
          'Tunjangan Makan',
          `Rp. ${
            presencesData[i].salary_groups[0].lunch_allowance
              ? formatCurrency(presencesData[i].salary_groups[0].lunch_allowance)
              : '0'
          }`,
        ...
        const secondHeaderContent = [
          ...
          'Tunjangan Transpor',
          `Rp. ${
            presencesData[i].salary_groups[0].transport_allowance
              ? formatCurrency(presencesData[i].salary_groups[0].transport_allowance)
              : '0'
          }`,
      ...
      await nodemailerMail.sendMail(
        {
          ...
          subject: `Data Presensi Periode ${start} s/d ${end} - ${
