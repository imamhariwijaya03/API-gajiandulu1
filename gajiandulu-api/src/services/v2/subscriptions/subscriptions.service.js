...
const subscriptionService = {
  ...
  updateSubscription: async (req, res) => {
    ...
    try {
      ...
      if (!isManager) {
        return res.status(403).json(response(false, 'Anda bukan manager'));
      }
      if (companyId.toString() !== isManager.company_id.toString()) {
        return res.status(403).json(response(false, 'ID Perusahaan berbeda'));
      }
      ...
      if (!subscribe) {
        journal = await JournalModel.create({
          ...
          description: `Tagihan berlangganan fitur ${subscribeData.subscribe_type}, durasi ${
            subscribeData.subscribe_freq
          } bulan. -- ${subscribeData.description}`
        });
        if (!journal) {
