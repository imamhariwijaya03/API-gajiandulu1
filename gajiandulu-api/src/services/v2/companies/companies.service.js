...
const companiesService = {
  createSalaryGroup: async (req, res) => {
    ...
    try {
      ...
      const description = `${checkuser.full_name} telah membuat golongan gaji dengan nama: ${
        salaryGroup.salary_name
      }`;
  ...
  editSalaryGroup: async (req, res) => {
    ...
    try {
      ...
      if (req.query.type === 'instant') {
        ...
        // SEND NOTIFICATION TO MANAGERS
        const description = `${checkUser.full_name} telah mengubah golongan gaji dengan nama ${
          salaryGroup.salary_name
        }`;
        observe.emit(EVENT.USER_ACTIVITY_NOTIF, {
          employeeId,
      ...
      await LogsModel.create({
        platform: 'app',
        company: salaryGroup.company.company_name,
        description: `${checkUser.full_name} telah mengubah golongan gaji dengan nama ${
          salaryGroup.salary_name
        }`
      });
  ...
  deleteSalaryGroup: async (req, res) => {
    ...
    try {
      ...
      // SEND NOTIFICATION TO MANAGERS
      const description = `${checkUser.full_name} telah menghapus golongan gaji dengan nama ${
        salaryGroup.salary_name
      }`;
      observe.emit(EVENT.USER_ACTIVITY_NOTIF, {
        employeeId,
