...
const divisionsService = {
      ...
      const division = await Divisions.findAll({
        ...
        include: {
          ...
          include: {
            model: EmployeeModel,
            attributes: ['id'],
            include: { model: UserModel, attributes: ['full_name', 'id'] }
          }
        }
      });
  ...
  deleteDivision: async (req, res) => {
    ...
    try {
      ...
      // SEND NOTIFICATION TO MANAGERS
      const checkUser = await UserModel.findOne({ where: { id } });
      const description = `${checkUser.full_name} telah menghapus divisi dengan nama ${
        division.name
      }`;
      observe.emit(EVENT.USER_ACTIVITY_NOTIF, {
        employeeId,
  ...
  editDivision: async (req, res) => {
    ...
    try {
      ...
      await Divisions.update({ name: data.name }, { where: { id: division_id } });
      ...
      // SEND NOTIFICATION TO MANAGERS
      const checkUser = await UserModel.findOne({ where: { id } });
      const description = `${checkUser.full_name} telah mengedit divisi dengan nama ${
        division.name
      }`;
      observe.emit(EVENT.USER_ACTIVITY_NOTIF, {
        employeeId,
