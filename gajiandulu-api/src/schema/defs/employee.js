...
const typeDef = gql`
  ...
  type Employee {
    ...
    totalWithdrawEmployee(month: String, year: String): Int
    restSalaryEmployee(month: String, year: String): Int
      ...
      try {
        ...
        const withdrawData = await JournalDetail.findAll({
          where: {
            ...
            $and: [
              Sequelize.where(
                Sequelize.fn('YEAR', Sequelize.col('journal_details.created_at')),
                params.year
              ),
              Sequelize.where(
                Sequelize.fn('MONTH', Sequelize.col('journal_details.created_at')),
                params.month
              )
            ]
          },
