...
const presenceOverdueCheck = async (checkin, employeeId) => {
  ...
  if (scheduleOnce) {
    todaySchedule = new Date(presenceDate + ' ' + scheduleOnce.shift.schedule_shift.start_time);
    ...
    if (scheduleContinous) {
      todaySchedule = new Date(
        presenceDate + ' ' + scheduleContinous.shift.schedule_shift.start_time
      );
      calculatePresence = Math.floor((checkin - todaySchedule) / (1000 * 60));
    } else {
