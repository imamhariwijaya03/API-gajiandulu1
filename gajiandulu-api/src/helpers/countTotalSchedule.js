...
const countTotalSchedule = async (employeeId, start, end) => {
  ...
  while (startDate <= endDate) {
    ...
    queries += `SELECT ${'`id`'} FROM ${'`schedule_templates`'} AS ${'`schedule_templates`'} WHERE (IF(${'`deleted_date`'}, ${'`deleted_date`'}, '2999-12-31') NOT LIKE '%${date}%' AND NOT (DATE_FORMAT(IF(${'`deleted_after`'}, ${'`deleted_after`'}, '2999-12-31'), '%Y-%m-%d') <= '${date}') AND NOT (DATE_FORMAT(IF(${'`end_repeat`'}, ${'`end_repeat`'}, '2999-12-31'), '%Y-%m-%d') <= '${date}') AND ${'`schedule_templates`'}.${'`employee_id`'} = ${employeeId} AND ((${'`schedule_templates`'}.${'`start_date`'} <= '${date}' AND ${'`schedule_templates`'}.${'`end_date`'} >= '${date}') OR ((${'`schedule_templates`'}.${'`start_date`'} <= '${date}' AND CASE
       ...
       END))))${startDate.getTime() !== endDate.getTime() ? ' UNION ALL ' : ''}`;
  ...
  return totalTemplated.length + totalDefined.length;
};
module.exports = countTotalSchedule;
